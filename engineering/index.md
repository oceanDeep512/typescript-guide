...
`fetchUser` 悬停看到的是 `Promise<User>`——**校验只发生在边界那一次**，之后整条链路都拿可信类型。
...
`leak` 是 `any`——一个 `any` 进去，`any` 出来，后面接什么都不会再报错。
...
`^?` 显示的是 `string`，但这是**用 `!` 强行骗来的**：运行时 `profile` 可能根本不存在。
...
// 而 schema 推导出来的会跟着变（多了 email: string）
...
| [Node 与服务端类型](./node) | 写后端（会用到上一篇的 schema 做边界校验） |
